...
var load = function(filename) {
    ...
    db.prepare('CREATE TABLE IF NOT EXISTS orders (id PRIMARY KEY, produit TEXT, FOREIGN KEY(id) REFERENCES users(id) ON DELETE CASCADE)').run();
	db.prepare('CREATE TABLE IF NOT EXISTS messages (idUser ID, message TEXT)').run();
	
   var insert = db.prepare("INSERT INTO Plantes VALUES (@id, @description, @image, @name, 10, 100)");
